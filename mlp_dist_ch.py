...
#dataset['hujan radar']=pd.to_numeric(dataset['hujan radar'],downcast='float')
print(dataset)
...
for d in range(10,210,10):
    mc0 = MLPRegressor(hidden_layer_sizes=d,activation='relu',solver='adam',alpha=0.0001,batch_size='auto',learning_rate='constant',max_iter=2000)
    model_mc0 = mc0.fit(input_train, output_train)
    mc_pred0 = model_mc0.predict(input_test)
    mse_mc=metrics.mean_squared_error(output_test, mc_pred0)
    MLP_hidden.append(mse_mc)
best_MLP=(MLP_hidden.index(min(MLP_hidden))+1)*10
...
rt = MLPRegressor(hidden_layer_sizes=best_MLP,activation='relu',solver='adam',alpha=0.0001,batch_size='auto',learning_rate='constant',max_iter=1000)
...
np.savetxt('stats_mlpreg.csv',[rmse,rmse_mod,skill_tc,bias],delimiter=',')

MLP_model='MLP_model.sav'
pickle.dump(rt,open(MLP_model,'wb'))
