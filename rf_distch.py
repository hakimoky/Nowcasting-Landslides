...
for d in range(10,210,10):
    rc0 = RandomForestRegressor(n_estimators=d,criterion='mse',max_depth=4,min_weight_fraction_leaf=0.01,max_features=None)
    model_rc0 = rc0.fit(input_train, output_train)
    rc_pred0 = model_rc0.predict(input_test)
    mse_rc=metrics.mean_squared_error(output_test, rc_pred0)
    rf_depth.append(mse_rc)
best_rf=(rf_depth.index(min(rf_depth))+1)*10
print('depth = ',best_rf)
...
rt = RandomForestRegressor(n_estimators=best_rf,criterion='mse',max_depth=4,min_weight_fraction_leaf=0.001,max_features=None)
...
np.savetxt('stats_rfreg.csv',[rmse,rmse_mod,skill_tc,bias],delimiter=',')

RF_model='RF_model.sav'
pickle.dump(rt,open(RF_model,'wb'))
